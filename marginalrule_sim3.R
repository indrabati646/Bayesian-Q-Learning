#Marginal Decision rules for Simulation setting 3
...
qlfun <- function(data){
  ...
  for(j in 1:200){
    for(i in 1:nrow(x)){
      mvd <- mvdc(copula=cop, margins=c("truncnorm","truncnorm","truncnorm","truncnorm"),
                  paramMargins = list(list(mean=2*u1[i]-0.2*x[i,1],sd=0.5,a=0,b=1),
                                      list(mean=1-u1[i],sd=0.5,a=0,b=1),
                                      list(mean=0.5*u1[i]+0.5*x[i,2],sd=0.5,a=0,b=1),
                                      list(mean=1.5*u1[i]+0.3*x[i,3],sd=0.5,a=0,b=1)))
      Ds1[i,,j] <- rMvdc(1,mvd)
    }
  }
  a11z <- a12z <- rep(0,n)
  for(j in 1:200){
    a11z <- a11z +ifelse(1.5*Ds1[,1,j]-0.8*Ds1[,2,j]-0.5*x[,2]>0,1,0)
    a12z <- a12z + ifelse(-1+Ds1[,3,j]+0.8*Ds1[,4,j]+0.4*x[,3]>0,1,0)
  }
  ...
  for(k in 1:200){
    ...
    for(l in 1:n){
      if(a11t[l,k]==1 & st[l,k]==1){
        vst_dst[l,k] <- 0.7+0.2*x[l,1]+0.2*(x[l,2]+Ds1[l,2,k])+2*(1.5*Ds1[l,1,k]-0.8*Ds1[l,2,k]-0.5*x[l,2])
      }
      if(a11t[l,k]==1 & st[l,k]==0 & a12t[l,k]==1){
        vst_dst[l,k] <- 0.7+0.2*x[l,1]+0.2*x[l,2]+0.2*Ds1[l,2,k]+2*(1.5*Ds1[l,1,k]-0.8*Ds1[l,2,k]-0.5*x[l,2])+0.5*x[l,3]+0.5*Ds1[l,4,k]+
          2*(Ds1[l,3,k]-0.8*Ds1[l,4,k]+0.4*x[l,3])
        
      }
      if(a11t[l,k]==1 & st[l,k]==0 & a12t[l,k]==0){
        vst_dst[l,k] <- 0.7+0.2*x[l,1]+0.2*x[l,2]+0.2*Ds1[l,2,k]+2*(1.5*Ds1[l,1,k]-0.8*Ds1[l,2,k]-0.5*x[l,2])+
          0.5*x[l,3]+0.5*Ds1[l,4,k]-2*(Ds1[l,3,k]-0.8*Ds1[l,4,k]+0.4*x[l,3])
  ...
  yf1 <- yf2 <- gg1 <- array(0,dim=c(n,mcmc-2,100))
  for(k in 1:100){
    for(i in 1:nrow(x)){
      for(j in 1:200){
        
        for(l in 1:(mcmc-2)){
          yf1[i,l,k] <- sum(par.y1[l+2,1:K1]*(par.y1[l+2,indyy1]+sum(c(x[i,1:2],unlist(Ds1[i,1:2,j]))* par.y1[l+2,indyy3])))
          yf2[i,l,k] <- sum(par.y2[l+2,1:K1]*(par.y2[l+2,indyy1]+sum(c(x[i,1:2],unlist(Ds1[i,1:2,j]))* par.y2[l+2,indyy3])))
          gg1[i,l,k] <- ifelse(yf1[i,l,k]>yf2[i,l,k],1,0)
  ...
  for(k in 1:200){
    ...
    for(l in 1:n){
      if(a11[l,k]==1 & smat[l,k]==1){
        vst_dhat[l,k] <- 0.7+0.2*x[l,1]+0.2*(x[l,2]+Ds1[l,2,k])+2*(1.5*Ds1[l,1,k]-0.8*Ds1[l,2,k]-0.5*x[l,2])
      }
      if(a11[l,k]==1 & smat[l,k]==0 & ggs[l]==1){
        vst_dhat[l,k] <- 0.7+0.2*x[l,1]+0.2*x[l,2]+0.2*Ds1[l,2,k]+2*(1.5*Ds1[l,1,k]-0.8*Ds1[l,2,k]-0.5*x[l,2])+
          0.5*x[l,3]+0.5*Ds1[l,4,k]+2*(Ds1[l,3,k]-0.8*Ds1[l,4,k]+0.4*x[l,3])
      }
  ...
  for(j in 1:200){
    a1t[,j] <- ifelse(1.5*Ds1[,1,j]-0.8*Ds1[,2,j]-0.5*x[,2]>0,1,0)
    a2t[,j] <- ifelse(Ds1[gind,3,j]-0.8*Ds1[gind,4,j]+0.4*x[gind,3]>0,1,0)
  }
...
result <- parLapply(cl, sim_3, qlfun)  
stopCluster(cl)  
